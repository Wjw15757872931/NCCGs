...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/paad")

data <- GDCprepare(query, directory = "./TCGA/paad")
save(data, file = "data/paad.Rdata")

load("data/paad.Rdata")
...
save(all_data, file = "data/paad_alldata.Rdata")


###
rm(list = ls())
load("data/paad_alldata.Rdata")
...
cg = c("ZFP36L1", "VGLL4", "TMSB10", "PPP1CB", "PERP", "PDIA3", "PABPC1", 
       "LGALS3", "GAPDH", "ELF3", "CANX", "B4GALT1")
...
save(all_data, file = "data/TPM_paad_alldata.Rdata")

rm(list = ls())
load("data/TPM_paad_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/PAAD_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/PAAD_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/paad_immune_cibersort.Rdata")

rm(list = ls())
load("data/paad_immune_cibersort.Rdata")
load("data/paad_alldata.Rdata")
