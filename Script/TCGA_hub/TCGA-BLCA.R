...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/blca")

data <- GDCprepare(query, directory = "./TCGA/blca")
save(data, file = "data/blca.Rdata")
...
save(all_data, file = "data/blca_alldata.Rdata")


###
rm(list = ls())
load("data/blca_alldata.Rdata")
...
save(all_data, file = "data/TPM_blca_alldata.Rdata")

rm(list = ls())
load("data/TPM_blca_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/BLCA_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/BLCA_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/blca_immune_cibersort.Rdata")

rm(list = ls())
load("data/blca_immune_cibersort.Rdata")
load("data/blca_alldata.Rdata")
