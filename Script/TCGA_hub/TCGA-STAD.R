...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/stad")

data <- GDCprepare(query, directory = "./TCGA/stad")
save(data, file = "data/stad.Rdata")
...
save(all_data, file = "data/stad_alldata.Rdata")


###
rm(list = ls())
load("data/stad_alldata.Rdata")
...
all_data$GADH11111 = (all_data$HSPB1 + all_data$FBLN1) / (2 * all_data$ARGLU1)
cg = c("GADH11111")
...
save(all_data, file = "data/TPM_stad_alldata.Rdata")

rm(list = ls())
load("data/TPM_stad_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/STAD_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/STAD_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/stad_immune_cibersort.Rdata")

rm(list = ls())
load("data/stad_immune_cibersort.Rdata")
load("data/stad_alldata.Rdata")
