...
data <- GDCprepare(query, directory = "./TCGA/luad")
save(data, file = "luad.Rdata")
...
save(all_data, file = "data/luad_alldata.Rdata")


###
rm(list = ls())
load("data/luad_alldata.Rdata")
...
all_data$GADH11111 = all_data$GAPDH / all_data$DHRS3
cg = c("GADH11111")
...
save(all_data, file = "data/TPM_luad_alldata.Rdata")

rm(list = ls())
load("data/TPM_luad_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/LUAD_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/LUAD_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/luad_immune_cibersort.Rdata")

rm(list = ls())
load("data/luad_immune_cibersort.Rdata")
load("data/luad_alldata.Rdata")
