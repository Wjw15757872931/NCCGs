...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/esca")

data <- GDCprepare(query, directory = "./TCGA/esca")
save(data, file = "data/esca.Rdata")
...
save(all_data, file = "data/esca_alldata.Rdata")


###
rm(list = ls())
load("data/esca_alldata.Rdata")
...
save(all_data, file = "data/TPM_esca_alldata.Rdata")

rm(list = ls())
load("data/TPM_esca_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/ESCA_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/ESCA_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/esca_immune_cibersort.Rdata")

rm(list = ls())
load("data/esca_immune_cibersort.Rdata")
load("data/esca_alldata.Rdata")
