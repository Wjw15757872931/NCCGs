...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/ucec")

data <- GDCprepare(query, directory = "./TCGA/ucec")
save(data, file = "data/ucec.Rdata")

load("data/ucec.Rdata")
...
save(all_data, file = "data/ucec_alldata.Rdata")


###
rm(list = ls())
load("data/ucec_alldata.Rdata")
...
all_data$GADH11111 = (all_data$RNF213 + all_data$PABPC1 + all_data$MGST1 + all_data$ARPC2 + all_data$ACTR3) / (5 * all_data$B4GALT1)
cg = c("GADH11111")
...
save(all_data, file = "data/TPM_ucec_alldata.Rdata")

rm(list = ls())
load("data/TPM_ucec_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/UCEC_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/UCEC_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/ucec_immune_cibersort.Rdata")

rm(list = ls())
load("data/ucec_immune_cibersort.Rdata")
load("data/ucec_alldata.Rdata")
