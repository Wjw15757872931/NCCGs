...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/lusc")

data <- GDCprepare(query, directory = "./TCGA/lusc")
save(data, file = "lusc.Rdata")

load("data/lusc.Rdata")
...
save(all_data, file = "data/lusc_alldata.Rdata")


###
rm(list = ls())
load("data/lusc_alldata.Rdata")
...
cg = c("TSC22D1", "TM9SF2", "RNF213", "HSD17B4", "HLA-E", "ERGIC1",
       "DNAJC3", "DHRS3", "CD74", "CANX", "B4GALT1", "ARPC2")
...
save(all_data, file = "data/TPM_lusc_alldata.Rdata")

rm(list = ls())
load("data/TPM_lusc_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/LUSC_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/LUSC_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/lusc_immune_cibersort.Rdata")

rm(list = ls())
load("data/lusc_immune_cibersort.Rdata")
