...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/kirp")

data <- GDCprepare(query, directory = "./TCGA/kirp")
save(data, file = "data/kirp.Rdata")

load("data/kirp.Rdata")
...
save(all_data, file = "data/kirp_alldata.Rdata")


###
rm(list = ls())
load("data/kirp_alldata.Rdata")

data = all_data[-c(1:4)][colnames(all_data[-(1:4)]) %in% sc_sn_genes$symbol]
...
cg = c("ZFP36L1", 'PABPC1', "IFI16", "FBLN1", "ACTR3")
...
save(all_data, file = "data/TPM_kirp_alldata.Rdata")

rm(list = ls())
load("data/TPM_kirp_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/KIRP_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/KIRP_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/kirp_immune_cibersort.Rdata")

rm(list = ls())
load("data/kirp_immune_cibersort.Rdata")
load("data/kirp_alldata.Rdata")
