...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/lihc")

data <- GDCprepare(query, directory = "./TCGA/lihc")
save(data, file = "data/lihc.Rdata")

load("data/lihc.Rdata")
...
save(all_data, file = "data/lihc_alldata.Rdata")


###
rm(list = ls())
load("data/lihc_alldata.Rdata")
...
cg = c("ZFP36L1", "VGLL4", "TPM3", "TMSB4X", "TMSB10", "RBM47", "PPP1CB", "PERP", 
       "PABPC1", "LGALS3", "GFPT1", "GAPDH", "ARPC2", "ACTR3")
...
save(all_data, file = "data/TPM_lihc_alldata.Rdata")

rm(list = ls())
load("data/TPM_lihc_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/LIHC_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/LIHC_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/lihc_immune_cibersort.Rdata")

rm(list = ls())
load("data/lihc_immune_cibersort.Rdata")
load("data/lihc_alldata.Rdata")
