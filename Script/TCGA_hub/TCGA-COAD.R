...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/coad")

data <- GDCprepare(query, directory = "./TCGA/coad")
save(data, file = "data/coad.Rdata")
...
save(all_data, file = "data/coad_alldata.Rdata")


###
rm(list = ls())
load("data/coad_alldata.Rdata")
...
save(all_data, file = "data/TPM_coad_alldata.Rdata")

rm(list = ls())
load("data/TPM_coad_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/COAD_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/COAD_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/coad_immune_cibersort.Rdata")

rm(list = ls())
load("data/coad_immune_cibersort.Rdata")
load("data/coad_alldata.Rdata")
