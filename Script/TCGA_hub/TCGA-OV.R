...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/ov")

data <- GDCprepare(query, directory = "./TCGA/ov")
save(data, file = "data/ov.Rdata")
...
save(all_data, file = "data/ov_alldata.Rdata")


###
rm(list = ls())
load("data/ov_alldata.Rdata")
...
all_data$GADH11111 = (all_data$ZFP36L1 + all_data$TPT1 + all_data$ELF3) / (3 * all_data$B4GALT1)
cg = c("GADH11111")
...
save(all_data, file = "data/TPM_ov_alldata.Rdata")

rm(list = ls())
load("data/TPM_ov_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/OV_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/OV_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/ov_immune_cibersort.Rdata")

rm(list = ls())
load("data/ov_immune_cibersort.Rdata")
load("data/ov_alldata.Rdata")
