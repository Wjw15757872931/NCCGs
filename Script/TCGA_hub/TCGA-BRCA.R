...
GDCdownload(query, method = "api", files.per.chunk = 100, directory="./TCGA/brca")

data <- GDCprepare(query, directory = "./TCGA/brca")
save(data, file = "brca.Rdata")

load("data/brca.Rdata")
...
save(all_data, file = "data/brca_alldata.Rdata")


###
load("data/brca_alldata.Rdata")
...
save(all_data, file = "data/TPM_brca_alldata.Rdata")

rm(list = ls())
load("data/TPM_brca_alldata.Rdata")

data = all_data[-c(1:4)]
data = as.data.frame(t(data))

write.table(data, file = "data/BRCA_TPM.txt", sep = "\t")
...
immune_cibersort <- CIBERSORT(
  ...
  mixture_file = "data/BRCA_TPM.txt",
  perm = 10, QN = F
)

save(immune_cibersort, file = "data/brca_immune_cibersort.Rdata")

rm(list = ls())
load("data/brca_immune_cibersort.Rdata")
cg = c("CD74", "B4GALT1", "ARGLU1")
